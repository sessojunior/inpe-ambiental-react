...
export default function TopBar() {
  ...
  const classButton = "size-9 md:size-[38px] inline-flex justify-center items-center gap-2 rounded-full font-medium text-gray-700 hover:bg-blue-200 text-xs md:text-sm"
  ...
  return (
    <header className="flex justify-start md:justify-between items-center px-4 w-full h-16 bg-gray-100 border border-y-gray-300">
      ...
      <div className="flex gap-1">
        ...
        <button className={`${config.quantityFrames === 4 ? classButtonActive : classButton} hidden lg:inline-flex`} onClick={() => handleQuantityFrames({ quantity: 4 })} title="Exibir 4 quadros na página"><BsBorderAll /></button>
      </div>
    </header>
  )
}
